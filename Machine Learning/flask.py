...
# Hapus titik dari string dan konversi menjadi integer untuk komputasi
df['Price_in_IDR'] = df['Price_in_IDR'].str.replace(".", "").astype(int)

# Tambahkan kolom tampilan dengan format angka bertitik
df['Price_in_IDR_Display'] = df['Price_in_IDR'].apply(lambda x: f"{x:,}".replace(",", "."))
...
def filter_price():
    try:
        ...
        results = filtered_data.to_dict(orient='records')  # Convert DataFrame to list of dictionaries
